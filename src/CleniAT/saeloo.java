...
public class saeloo {
	...
	public static void sealo(Player p) {
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání mail zprávy hráči Saelo").setClickEvent(ClickAction.SUGGEST_COMMAND, "/mail send Saelo ").send(p);
				p.playSound(p.getLocation(), Sound.BLOCK_WOODEN_BUTTON_CLICK_ON, 5, 1);
				b.close();
			}
		});
