...
public class murreder {

	public static void murdi(Player p) {
		GUICreatorAPI b = TheAPI.getGUICreatorAPI(p);
		b.setSize(27);
		b.setTitle("&e&lBloodmmureder");
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání soukromé zprávy hráči Bloodmmureder").setClickEvent(ClickAction.SUGGEST_COMMAND, "/msg Bloodmmureder ").send(p);
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání mail zprávy hráči Bloodmmureder").setClickEvent(ClickAction.SUGGEST_COMMAND, "/mail send Bloodmmureder ").send(p);
				p.playSound(p.getLocation(), Sound.BLOCK_WOODEN_BUTTON_CLICK_ON, 5, 1);
				b.close();
			}
		});
