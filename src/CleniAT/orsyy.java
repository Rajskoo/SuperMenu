...
public class orsyy {
	...
	public static void torsy(Player p) {
		GUICreatorAPI b = TheAPI.getGUICreatorAPI(p);
		b.setSize(27);
		b.setTitle("&e&lTheOrsy");
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání soukromé zprávy hráči TheOrsy").setClickEvent(ClickAction.SUGGEST_COMMAND, "/msg TheOrsy ").send(p);
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání mail zprávy hráči TheOrsy").setClickEvent(ClickAction.SUGGEST_COMMAND, "/mail send TheOrsy ").send(p);
				p.playSound(p.getLocation(), Sound.BLOCK_WOODEN_BUTTON_CLICK_ON, 5, 1);
				b.close();
			}
		});
