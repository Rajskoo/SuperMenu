...
public class hoff {
	...
	public static void hoffik(Player p) {
		...
		ops.put(Options.RUNNABLE, new Runnable() {
			public void run() {
				new HoverMessage("&6Klikni pro poslání mail zprávy hráči Hoff").setClickEvent(ClickAction.SUGGEST_COMMAND, "/mail send Hoff ").send(p);
				p.playSound(p.getLocation(), Sound.BLOCK_WOODEN_BUTTON_CLICK_ON, 5, 1);
				b.close();
			}
		});
